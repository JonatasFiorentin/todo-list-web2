      ...
      <li className="flex items-center justify-between p-4 bg-gray-100 rounded-lg shadow-sm hover:bg-gray-200 transition-colors">
        <div className="flex items-center space-x-3">
          <input
            ...
            aria-label={`Marcar como ${todo.marcado ? 'não concluído' : 'concluído'} ${todo.text}`}
          />
          <label
            className={`text-base font-medium ${todo.marcado ? 'line-through text-gray-500' : 'text-gray-800'}`}
          >
            {todo.text}
          </label>
        </div>
